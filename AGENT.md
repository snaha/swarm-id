...
**Key Innovation**: Popup-based authentication flow using shared localStorage. In production (secure context), storage works immediately for Chrome/Firefox. On localhost, Chrome/Firefox can request shared storage access via Storage Access API (requires clicking iframe button first). Safari requires disabling cross-site tracking prevention ([#167](https://github.com/snaha/swarm-id/issues/167)). Safari private mode works with ITP (storage partitioning) disabled, but sessions are ephemeral (lost when the private window closes).
