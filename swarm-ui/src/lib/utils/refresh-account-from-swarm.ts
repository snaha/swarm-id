...
export async function refreshAccountFromSwarm(accountId: string): Promise<RefreshResult> {
  ...
  if (!account) {
    return { ok: false, error: 'No local account for this id' }
  }
  ...
  try {
    ...
    if (!refBytes) {
      return {
        ok: false,
        error: `No account backup found at ${bee.url}`,
      }
    }
    ...
    console.log(
      `[RefreshAccount] devices=${snapshot.metadata.devices.length} activeDevices=${snapshot.metadata.activeDevices.length} bee=${bee.url}`,
    )
    ...
    accountsStore.applyRefreshedSnapshot(ethAddress, {
      ...
      activeDevices: snapshot.metadata.activeDevices,
    })
