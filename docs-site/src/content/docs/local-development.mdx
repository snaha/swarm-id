...
The Identity UI includes a Developer Tools page at `http://localhost:5510/dev` with utilities for testing and debugging.
...
│ id.yourdomain.com       │           │   5510 → identity   │
│   → 127.0.0.1:5510      │           │   (Vite dev server) │
└─────────────────────────┘           └─────────────────────┘
```

### VPS Setup (one-time)

1. Add nginx server blocks pointing to `127.0.0.1:18080` (demo) and `127.0.0.1:5510` (identity)
...
VITE_ADDITIONAL_SERVER_ALLOWED_HOSTS=id.yourdomain.com pnpm dev:ui:legacy

# Terminal 3: Open SSH tunnel
ssh -R 18080:localhost:3000 -R 5510:localhost:5510 user@your-vps
```
