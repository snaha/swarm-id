...
Safari's Intelligent Tracking Prevention (ITP) partitions storage for third-party iframes, so the proxy cannot read the trusted domain's localStorage. Uploads still work — the connect popup hands the iframe the account's stamps and signer keys — but nothing is persisted in the partition, so every page load re-runs the handshake. See [issue #277](https://github.com/snaha/swarm-id/issues/277) for the background.
