...
Swarm ID uses an OAuth-style popup authentication flow. Chrome and Firefox work out of the box; where the browser partitions the proxy iframe's storage — Safari's Intelligent Tracking Prevention, or strict privacy settings elsewhere — the connect popup hands the iframe the account's upload credentials directly, so uploads keep working. See [#277](https://github.com/snaha/swarm-id/issues/277) for the background.
