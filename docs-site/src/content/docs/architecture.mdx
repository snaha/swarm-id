...
Swarm ID uses an OAuth-style popup authentication flow. Chrome and Firefox work out of the box; Safari works in download-only mode (auth works, uploads disabled due to ITP storage partitioning). See [#167](https://github.com/snaha/swarm-id/issues/167) for details.
