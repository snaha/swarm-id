...
Swarm ID uses an OAuth-style popup authentication flow. The proxy iframe reads the trusted domain's first-party store whenever the embedding page is **same-site** with it (ports do not affect _site_, so `swarm-demo.snaha.net` embedding `swarm-id.snaha.net` is same-site, as is anything on `localhost`). Where the two are genuinely cross-site, or the browser partitions regardless — Safari's Intelligent Tracking Prevention, or strict privacy settings elsewhere — the connect popup hands the iframe the account's upload credentials directly, so uploads keep working. See [#277](https://github.com/snaha/swarm-id/issues/277) for the background.

:::note[Confirmed on real Safari]
The whole path **is** confirmed on real Safari (iOS 18.7 / Safari 26.6): ITP partitions the iframe, the connect popup's `postMessage` through `window.opener` reaches it, the session builds a working stamper from the handed-over view, and a chunk uploads and reads back byte-identical. A handover that carries no account view is refused rather than establishing a session that could neither upload nor be told it had been revoked, and `ConnectionInfo.uploadUnavailableReason` says which of the remaining causes a connected session hit.

A private window passes the same checks — sessions there are still discarded when the window closes, by design. Unrun: whether a dormant account's partitioned storage survives ITP's ~30-day eviction window ([#570](https://github.com/snaha/swarm-id/issues/570)).
:::
...
Where the browser also partitions the iframe away from the trusted domain's own storage (Safari's ITP, or any cross-site embedding), the connect popup hands the iframe the account's synced projection directly and the iframe keeps it under its own partition. It is a full writer there, and a peer on the [account bus](/account-bus), which is how a revoke or a rename made anywhere else still reaches it while it is open.
...
The [account bus](/account-bus) has its own message kinds — account snapshots, partition handover, presence — that travel between the account's live contexts rather than between a page and its iframe.

## Backup & Recovery

Swarm ID supports account backup via encrypted `.swarmid` files, automatic Swarm-based sync, and live propagation between open contexts over the [account bus](/account-bus).
