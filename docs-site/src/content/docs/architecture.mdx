...
:::caution[Safari uploads are not confirmed yet]
The credential handover **is** confirmed on real Safari (iOS 18.7 / Safari 26.6): ITP partitions the iframe, and the connect popup's `postMessage` through `window.opener` reaches it, so the session authenticates on the partitioned path. The upload that follows has still only run on Chromium and Firefox with third-party storage partitioned, so treat Safari upload support as expected rather than confirmed. A session where the handover fails degrades to download-only rather than erroring, and `ConnectionInfo.uploadUnavailableReason` says which of the causes it hit.
:::
