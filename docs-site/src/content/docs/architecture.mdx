...
:::caution[Safari support is unverified]
The partitioned write path is verified on Chromium and Firefox with third-party storage partitioned, but not yet on real Safari. Until it is, treat Safari upload support as expected rather than confirmed — a session where the handover fails degrades to download-only rather than erroring.
:::
