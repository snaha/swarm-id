...
| `uploadUnavailableReason`   | `"no-stamp" \| "stamper-failed" \| "stamp-expired" \| undefined` | Why `uploadMode` is `"unavailable"`; absent whenever uploads work. See [Why Uploads Are Unavailable](../subsidised-gateway/#why-uploads-are-unavailable)                                |
...
Uploads a file to the Swarm network, wrapped in a manifest that carries its name and content type. The reference is read back with [`downloadFile`](#downloadfilereference-path-options-requestoptions), or from a gateway at `/bzz/<reference>/`.
...
// served at https://<gateway>/bzz/<reference>/
...
import type {
  ...
  // Account bus (identity UI internals, below)
...
:::caution[Not a dApp API from here on]
Everything below this note is imported by the identity UI (the trusted domain) from `@snaha/swarm-id`, and documented here because the package exports it. **A dApp has no use for it and cannot call it**: every function takes the account record, the master key or `derivationKey`, and those never leave the trusted domain. A dApp holds a `SwarmIdClient`, and no message on the wire hands any of them out. The `.swarmid` backup file is written and read by the identity UI alone; see [Architecture](../architecture/#backup-and-recovery).
:::

## Account Bus Delta

Used by the identity UI. The [account bus](../account-bus/) is how one account's live contexts (the SwarmID tab, proxy iframes across partitions, other devices) hear about a change before the next Swarm feed read. The identity UI publishes an `account-delta` after every account mutation it makes; the proxy iframe publishes its own from inside the library.

### `accountDeltaSnapshot(account)`

Build the `snapshot` field of an `account-delta` bus message from the account record as the identity UI stores it. The result is the record's state with the per-app session fields removed: `appSecret` and `connectedUntil` on each connected app. They are stripped because one receiver is a proxy iframe embedded by another dApp's page, which must not learn a secret it did not derive itself. The proxy strips the same fields on its side, so the rule exists once.

**Parameters:**

| Parameter | Type            | Description                                                                                       |
| --------- | --------------- | ------------------------------------------------------------------------------------------------- |
| `account` | `SyncedAccount` | The account record from the identity UI's account store, the shape `SyncedAccountSchemaV1` parses |

**Returns:** `AccountStateSnapshot` with the session fields stripped, ready to send as `{ type: 'account-delta', snapshot }`

```typescript
import { accountDeltaSnapshot } from '@snaha/swarm-id'
import type { SyncedAccount } from '@snaha/swarm-id'

function publish(account: SyncedAccount) {
...
```typescript
// The identity UI's stored account record (`SyncedAccountSchemaV1`): the part
// of an account that is the same on every device. Device-local state (the
// seed vault, the access methods) is kept beside it, never in it.
interface SyncedAccount {
  id: EthAddress // account id
  name: string
  createdAt: number
  derivationKey: string // 64-char hex; every sub-key derives from it
  publicKey: string // compressed secp256k1, shown to dApps as the identity key
  defaultPostageStampBatchID?: BatchId
  devices: Device[]
  connectedApps: ConnectedApp[]
  postageStamps: PostageStamp[]
  settings?: { appSessionDuration?: number }
  accountNameAt?: number // per-field last-writer-wins clocks
  defaultStampAt?: number
  settingsAt?: number
  lastModified?: number
  partitionCount?: number // postage-batch slot partitions; absent = 1
}

// What an `account-delta` carries, and what a receiver folds into its own
// account record with the same last-writer-wins rules as a Swarm feed payload
interface AccountStateSnapshot {
  version: 1
  timestamp: number
  accountId: string
  metadata: AccountMetadata // name, default stamp, public key, settings, devices, clocks
  connectedApps: ConnectedApp[]
  postageStamps: PostageStamp[]
}
```
...
Used by the identity UI. The full tree, with every context string, is on [Key Derivation](../key-derivation/). Every function below takes and returns 64-character hex strings unless noted.

| Function                                  | Derives                                                                       |
| ----------------------------------------- | ----------------------------------------------------------------------------- |
| `deriveSecret(keyHex, context)`           | `HMAC-SHA256(bytes(keyHex), utf8(context))`, the primitive under all of these |
| `deriveAccountDerivationKey(masterKey)`   | `derivationKey`, the key stored on the account                                |
| `deriveSwarmEncryptionKey(derivationKey)` | the key that encrypts everything the account writes to Swarm                  |
| `derivePostageSignerKey(derivationKey)`   | the key that owns and stamps with the account's postage batches               |
| `deriveSharingKey(derivationKey)`         | `{ secret, publicKey }`, the account-wide ACT sharing key (synchronous)       |
| `deriveBusContext(derivationKey)`         | `{ topic, encryptionKey }` for the account bus                                |
