...
Gets information about the Bee node configuration. Needs a real Bee node: the [public gateway](../public-gateway/) has no `/node` endpoint and this call fails there. Do not gate uploads on it; its purpose is to detect a dev-mode node, where deferred uploads are required.
