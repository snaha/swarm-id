...
Every account has an avatar. Today it is generated from the account's identity id — the same image Swarm ID shows for that account, so the user recognises it in both places. Accounts will be able to upload their own, so treat the avatar as an image at a URL rather than as markup you can regenerate.
...
| Property | Type          | Description                                                                         |
| -------- | ------------- | ----------------------------------------------------------------------------------- |
| `source` | `"generated"` | Where the image came from. `generated` is the fallback derived from the identity id |
| `url`    | `string`      | Renderable as an `<img>` source; a `data:` URL for generated avatars                |

Check `source` rather than assuming an inline SVG — an uploaded avatar is a bitmap fetched from Swarm.

### `getAvatar(options?)`

The avatar at a specific size. Most apps do not need this — `identity.avatar` is already resolved and, being vector today, scales to any size with CSS. Reach for it when you need the image rendered at a particular pixel size; it is async because an uploaded avatar has to be re-fetched at that size.
