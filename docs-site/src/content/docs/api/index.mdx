...
After disconnection, the user will need to re-authenticate to perform uploads or access identity-related features. The `onConnectionChange` callback will fire with an un-authenticated snapshot (no `identity`, `canUpload: false` unless a subsidised gateway is configured).
...
Guaranteed to be defined after `initialize()` resolves.

**Type:** `ConnectionInfo | undefined`
