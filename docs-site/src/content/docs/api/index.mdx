...
An account can hold several stamps (each shown as a "drive" in the identity UI), so this returns all of them — letting an app reach a specific stamp (e.g. to upload with `UploadOptions.batchID`). With no `batchID`, uploads use the account's resolved default stamp.
