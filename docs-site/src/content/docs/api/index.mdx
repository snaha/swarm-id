...
Every account has an avatar. Today it is generated from the account's identity id — the same image Swarm ID shows for that account, so the user recognises it in both places. Accounts will be able to upload their own, which is why the client method is async and returns a URL rather than markup.

### `getAvatar(options?)`

Avatar of the connected identity, ready to render.
