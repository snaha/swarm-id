...
| `downloadReference(options?)` | Returns the reference for timestamp `options.at` |
| `downloadPayload(options?)` | Downloads payload data for timestamp `options.at` |
...
| `encryptionKey` | `string` | No | Encryption key (hex) for encrypted feed updates |
...
| `downloadReference(options?)` | Returns the reference for timestamp `options.at` |
| `downloadPayload(options?)` | Downloads payload data for timestamp `options.at` |
| `uploadPayload(data, options?)` | Uploads payload data and updates the feed |
| `uploadReference(reference, options?)` | Updates the feed with a reference |
...
**Returns (`uploadPayload` / `uploadReference`):**
