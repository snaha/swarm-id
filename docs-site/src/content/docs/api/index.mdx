...
### `createEncryptedExport(account, swarmEncryptionKeyHex)`

Create a fully encrypted `.swarmid` export object. Serializes the account's state (connected apps, postage stamps, metadata), derives an AES-GCM-256 key from the `swarmEncryptionKey` (itself derived from the account's stored `derivationKey`), encrypts the payload, and builds a header with the account name and export time. App connections are kept in the encrypted payload so that a restored account keeps them.

**Parameters:**

| Parameter               | Type              | Description                                                                                            |
| ----------------------- | ----------------- | ------------------------------------------------------------------------------------------------------ |
| `account`               | `SignedInAccount` | The signed-in account to export                                                                        |
...
const exported = await createEncryptedExport(account, swarmEncryptionKey)
...
if (result.success) {
  const { connectedApps, postageStamps } = result.data
}
```

### `parseEncryptedExportHeader(data)`

Parse and validate just the encrypted export header without decrypting. Useful for reading the account name and export date before attempting decryption.
...
| `input.connectedApps` | `ConnectedApp[]`  | Connected apps     |
...
Deserialize and validate an account state snapshot against the Zod schema.
...
Rebuild account state from Swarm on a device that has never seen the account. Derives the keys, reads the account's device roster, reads every device's latest state feed, and folds them into one snapshot; see [Postage Batch Sharing](../multi-device-postage-batches/#account-state-sync).
...
The full tree, with every context string, is on [Key Derivation](../key-derivation/). Every function below takes and returns 64-character hex strings unless noted.
...
| `deriveBackupEncryptionKey(swarmEncryptionKey)` | the AES-GCM-256 `CryptoKey` for `.swarmid` files                              |
| `backupKeyToPrivateKey(backupKeyHex)`           | the backup signer as a bee-js `PrivateKey`                                    |
