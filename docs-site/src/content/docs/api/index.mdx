...
import type {
  ...
  // Account state (identity UI internals, below)
...
:::note[Identity UI internals from here on]
The rest of this page is what the identity UI (the trusted domain) imports from `@snaha/swarm-id`. It is exported because the UI is a package like any other, not because a dApp is meant to call it: every function below takes the account object, the master key or `derivationKey`, and those never leave the trusted domain. A dApp only ever holds a `SwarmIdClient`, and no message on the wire hands any of them out. The `.swarmid` backup file is written and read by the identity UI alone; see [Architecture](../architecture/#backup-and-recovery).
:::

## Account State Snapshots

The account-state snapshot is what the identity UI publishes over the [account bus](../account-bus/) as an `account-delta`, and what the sync coordinator captures before it writes this device's state feed. The serializers behind it are internal to the library.

### `accountToStateSnapshot(account, accountId, timestamp)`

Assemble a snapshot from a synced account record. `timestamp` becomes both the envelope timestamp and `metadata.lastModified`; a publisher passes `Date.now()`, a caller re-reading an unchanged record passes that record's own `lastModified`.

**Returns:** `AccountStateSnapshot`
