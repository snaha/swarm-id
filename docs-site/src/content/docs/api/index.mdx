...
The account bus carries an `account-delta`: the account's state as one snapshot, folded last-writer-wins by every live context of the account. The identity UI publishes one and receives them; the proxy iframe does the same on its own.

### `accountDeltaSnapshot(account)`

The wire form of a synced account record for an `account-delta` message: the snapshot with the per-app session fields (`appSecret`, `connectedUntil`) stripped, since the receivers include proxy iframes embedded by other dApps.

**Returns:** the `snapshot` field of an `account-delta` bus message

### Types

```typescript
// What a received `account-delta` carries
