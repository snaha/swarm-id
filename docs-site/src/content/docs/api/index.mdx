...
Epoch feed methods provide time-indexed updates backed by Single Owner Chunks (SOCs). Feed updates are addressed by `(topic, owner)` and can be looked up by timestamp.
...
| `findAt(at, after?)` | Finds the feed update valid at timestamp `at` |

**Returns (`findAt`):** `Promise<string | undefined>` - Swarm reference (hex string) or `undefined` if not found
...
const reference = await reader.findAt(BigInt(Date.now() / 1000))
```
...
| `getOwner()` | Resolves the feed owner address |
| `findAt(at, after?)` | Finds the feed update valid at timestamp `at` |
| `update(at, reference)` | Updates the feed at timestamp `at` with `reference` |

**Returns (`update`):** `Promise<string>` - SOC address/reference (hex string)

**Example:**

```typescript
const writer = client.makeEpochFeedWriter({ topic })
const socAddress = await writer.update(BigInt(Date.now() / 1000), reference)
const latest = await writer.findAt(BigInt(Date.now() / 1000))
```
