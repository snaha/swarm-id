...
<div class="space-y-2">
  ...
  >
    ...
    {:else if clientStore.uploadMode === 'subsidised'}
      <Badge variant="success" class="text-xs px-2 py-0.5">Subsidised</Badge>
    {:else}
  ...
  {#if expanded}
    ...
    {:else if clientStore.uploadMode === 'subsidised'}
      <div class="text-xs text-muted-foreground pl-6">Using public gateway for uploads</div>
    {/if}
  {/if}
</div>
