...
export default async function setup({ provide }: GlobalSetupContext) {
  if (!(await isClusterReachable())) {
    console.warn(
      "[cluster] No local Bee cluster reachable at http://localhost:1633 — cluster integration tests will be skipped. Run `pnpm dev:cluster start --full 4` to enable them.",
    )
    return
  }
