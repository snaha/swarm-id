...
const THIRD_DEVICE_ID = "device-third-333"
...
function makeActiveDevice(deviceId: string, partition: number): ActiveDevice {
...
function makeSnapshot(overrides: {
  ...
  return {
    ...
    metadata: {
      ...
      activeDevices: overrides.activeDevices ?? [],
...
describe("mergeSnapshotWithRemote — activeDevices union", () => {
  it("local self entry always wins for the current device", () => {
    const local = makeSnapshot({
      activeDevices: [makeActiveDevice(SELF_DEVICE_ID, 2)],
    })
    const remote = makeSnapshot({
      activeDevices: [makeActiveDevice(SELF_DEVICE_ID, 0)],
    })
    const result = mergeSnapshotWithRemote(local, remote, {
      selfDeviceId: SELF_DEVICE_ID,
    })
    const self = result.metadata.activeDevices.find(
      (a) => a.deviceId === SELF_DEVICE_ID,
    )
    expect(self?.partition).toBe(2)
  })

  it("includes remote entries for other devices", () => {
    const local = makeSnapshot({
      activeDevices: [makeActiveDevice(SELF_DEVICE_ID, 0)],
    })
    const remote = makeSnapshot({
      activeDevices: [
        makeActiveDevice(SELF_DEVICE_ID, 0),
        makeActiveDevice(OTHER_DEVICE_ID, 1),
      ],
    })
    const result = mergeSnapshotWithRemote(local, remote, {
      selfDeviceId: SELF_DEVICE_ID,
    })
    const ids = result.metadata.activeDevices.map((a) => a.deviceId).sort()
    expect(ids).toEqual([OTHER_DEVICE_ID, SELF_DEVICE_ID].sort())
  })

  it("drops a non-self remote entry that collides with self's partition", () => {
    const local = makeSnapshot({
      activeDevices: [makeActiveDevice(SELF_DEVICE_ID, 0)],
    })
    const remote = makeSnapshot({
      activeDevices: [makeActiveDevice(OTHER_DEVICE_ID, 0)], // collides
    })
    const result = mergeSnapshotWithRemote(local, remote, {
      selfDeviceId: SELF_DEVICE_ID,
    })
    expect(result.metadata.activeDevices).toEqual([
      makeActiveDevice(SELF_DEVICE_ID, 0),
    ])
  })

  it("preserves local non-self entries that aren't in remote", () => {
    const local = makeSnapshot({
      activeDevices: [
        makeActiveDevice(SELF_DEVICE_ID, 0),
        makeActiveDevice(OTHER_DEVICE_ID, 1),
    ...
    const remote = makeSnapshot({
      activeDevices: [makeActiveDevice(THIRD_DEVICE_ID, 2)],
    })
    const result = mergeSnapshotWithRemote(local, remote, {
      selfDeviceId: SELF_DEVICE_ID,
    })
    const ids = result.metadata.activeDevices.map((a) => a.deviceId).sort()
    expect(ids).toEqual(
      [SELF_DEVICE_ID, OTHER_DEVICE_ID, THIRD_DEVICE_ID].sort(),
    )
  })
})
