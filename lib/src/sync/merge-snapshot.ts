 ...
 * state" stomps anything a peer just wrote (iteration-1 doc § Race 3 /
 * activeDevices snapshot stomp). The fix is to fold the remote snapshot
 * into the local one and publish the union.
 *
 * Strategy per field:
 *
 * - `devices`: union by `deviceId`. On conflict prefer the entry with the
 *   larger `lastSignedInAt` (recent sign-ins win).
 * - `activeDevices`: union by `deviceId`. The local self entry always
 *   wins for the current device — we know our own partition for sure,
 *   even if Swarm shows something stale. For other devices, take whatever
 *   the remote says. If two devices both claim the same partition, the
 *   local self entry takes priority and the remote's claim is dropped.
...
export interface MergeSnapshotOptions {
  /** This device's deviceId. The local entry for this device always wins. */
  selfDeviceId: string
}
...
export function mergeSnapshotWithRemote(
  ...
  const metadata: AccountMetadata = {
    ...
    activeDevices: mergeActiveDevices(
      local.metadata.activeDevices,
      remote.metadata.activeDevices,
      opts.selfDeviceId,
    ),
  }
...
function mergeActiveDevices(
  local: ActiveDevice[],
  remote: ActiveDevice[],
  selfDeviceId: string,
): ActiveDevice[] {
  const localSelf = local.find((a) => a.deviceId === selfDeviceId)
  const byDevice = new Map<string, ActiveDevice>()
  // Seed with remote.
  for (const a of remote) byDevice.set(a.deviceId, a)
  // Local non-self entries layer on top of remote.
  for (const a of local) {
    if (a.deviceId === selfDeviceId) continue
    byDevice.set(a.deviceId, a)
  }
  // Local self is the source of truth for our own partition.
  if (localSelf) byDevice.set(selfDeviceId, localSelf)

  // Drop any non-self entry that collides with self's partition.
  if (localSelf) {
    for (const [deviceId, entry] of Array.from(byDevice.entries())) {
      if (
        deviceId !== selfDeviceId &&
        entry.partition === localSelf.partition
      ) {
