...
describe("createSyncAccount", () => {
  ...
  it("refuses to sync when the device isn't active for a multi-device account", async () => {
    const stores = createMockStores()
    // Multi-device account, but this device has no activeDevices entry.
    ...
    const multiDeviceAccount = {
      ...baseAccount,
      partitionCount: 2,
      activeDevices: [], // self not present → inactive
    }
