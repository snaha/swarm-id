...
export default [
  ...
  // Phase 2: the two package entries in ONE build (embeds worker code via
  // virtual module). One build, not two, so a module both entries reach is
  // emitted once, in a shared chunk: a class like `TimeoutError` has a single
  // identity whichever entry a consumer imports it from.
  {
    input: {
      "swarm-id": "src/index.ts",
