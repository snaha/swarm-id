...
Both create paths raise **`CreatePendingError`** when their confirmation wait times out, carrying
the `batchId` the chain has already named along with the transaction hash. It is the one failure
that arrives with a paid-for batch in hand, and a plain timeout threw the id away with it: nothing
persists it and no re-scan looks for it, so the batch was unreachable and the dialog's Try again
bought a second one. `runPurchase` records it before the error surfaces (§Recording below), turning
the worst case into a batch that may not exist rather than money that does not either. Only a
timeout — a wait that fails because the transaction **reverted** bought nothing, and recording that
would attach a batch the chain never made. Deriving the id up front instead is not an option here:
batch ids are `keccak256(creator, nonce)` and this repo deliberately never computes one.
...
| Confirmation timeout          | "Still pending" — resolved by reconciliation on next open. A purchase records its batch first (`CreatePendingError`). |
