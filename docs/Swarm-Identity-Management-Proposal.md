...
Status: **historical.** This is the original PoC proposal, first committed 2025-11-25; what it proposed as option 1 (trusted domain + iframe), app-specific keys derived from the master key and the app domain, and high-stakes keys never handed to apps are what we built, while its "start with local only" storage plan and its session keys are not — accounts sync to Swarm today ([`Account-State.md`](./Account-State.md)) and no session keys exist. For the current architecture see [`AGENTS.md`](../AGENTS.md) and the Architecture page on the docs site (`docs-site/src/content/docs/architecture.mdx`).
