...
/**
 * The create transaction is out and the chain has already named the batch it
 * made, but the confirmation wait ran out.
 *
 * Typed so the purchase can still be RECORDED: nothing else persists the batch
 * id and no re-scan looks for it, and a retry creates a second batch with a
 * fresh nonce. `reconcileStampFromChain` trues the record up on the next open,
 * and a transaction that never lands leaves a record the user can remove.
 */
export class CreatePendingError extends Error {
  readonly batchId: string
  readonly transactionHash: string

  constructor(batchId: string, transactionHash: string, cause: unknown) {
    super(
      'Your purchase was sent but not confirmed in time. The drive will appear once the chain confirms it — reopen the drive list to check, and do not pay again.',
      { cause },
    )
    this.name = 'CreatePendingError'
...
/**
 * Wait for a create transaction that has already yielded a batch id, keeping
 * the id attached to a timeout. Only a timeout: a wait that fails because the
 * transaction REVERTED bought nothing, and recording that would attach a batch
 * the chain never made.
 */
async function awaitCreated(
  chain: MultichainClient,
  created: { transactionHash: `0x${string}`; batchId: string },
): Promise<string> {
  try {
    await withTimeout(
    ...
    if (caught instanceof TimeoutError) {
      throw new CreatePendingError(created.batchId, created.transactionHash, caught)
    }
    throw caught
  }
...
export async function bundledCreate(
  ...
  return awaitCreated(chain, created)
}
