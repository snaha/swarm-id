 ...
 * so it is offered ahead of the bridged chains rather than as a fallback. It
 * used to go through Relay like everything else, which paid a cross-chain
 * network to move money across zero chains.
 *
...
export async function resolveGnosisDirectRail(): Promise<PaymentRail | undefined> {
  ...
  const chain = gnosisChain(identity.isMainnet)
  return {
    chains: [chain],
