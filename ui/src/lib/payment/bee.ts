...
/**
 * The one drive operation that still needs a Bee node: proving a batch can
 * actually stamp uploads. Paid operations (top-up, resize) go straight to the
 * PostageStamp contract instead — see payment/postage-onchain.ts. The node URL
 * defaults to the shared network setting (the same one the lib proxy and /dev
 * honour), so pointing the app at a different Bee node redirects this too.
 */
