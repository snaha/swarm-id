...
import { combineRails } from '$lib/payment/resolve-rail'
...
describe('combineRails', () => {
  ...
  /**
   * The regression this closes. Dispatch used to be per CHAIN, so the direct
   * rail — offered first because paying from Gnosis needs no bridge — claimed
   * all of Gnosis. It moves native xDAI only, so Gnosis USDC silently vanished
