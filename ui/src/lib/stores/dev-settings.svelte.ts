...
// /dev). It is off until switched on, and then sticks until switched off again.
...
// OFF by default, deliberately — not `import.meta.env.DEV`. The e2e suites buy
// real drives against the local chain, and a mock that defaulted on in dev
// would silently reroute every one of them through the widget.
function loadMockEnabled(): boolean {
  return browser && localStorage.getItem(MOCK_ENABLED_KEY) === 'true'
}
